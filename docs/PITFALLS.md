...
### S5. Storage objects are removed through the Storage API, never SQL

M4c's first design deleted a progress photo's objects inside
`delete_progress_photo()` and swept orphans with `pg_cron`. The live project has
`storage.protect_delete`, a statement trigger that raises `Direct deletion from
storage tables is not allowed. Use the Storage API instead.` — and bypassing it
(`storage.allow_delete_query`) deletes only the metadata row, leaving the blob in
S3 with nothing pointing at it.

**Rule:** remove objects with `storage.from(bucket).remove()`, which needs a
SELECT **and** a DELETE policy, while the row that authorises those policies
still exists; then delete the row. Anything left behind (a failed remove, an
upload whose row never landed) is swept by a service-role route
(`/api/cron/photo-orphans`), not by SQL.

### S6. A Storage policy that must exclude admins cannot delegate to the table's RLS

`progress_photos`' SELECT policy admits admins so `/admin` can count photos. A
`storage.objects` SELECT policy that said "the caller can see the row" would
have let every admin sign a URL for every client's body photo — the one thing
spec D9 rules out.

**Rule:** write the Storage predicate out in its own SECURITY DEFINER helper
(`can_view_progress_photo_object()`) naming exactly who may sign, and assert the
admin case in the harness (`an admin cannot sign the photo`).
...
### V3. Typed routes come from a gitignored file that goes stale

After adding `/(app)/body/...`, `tsc` failed on every route push in the app —
including M4b's untouched `/(app)/sync-queue`. The code was fine:
`apps/mobile/.expo/types/router.d.ts` is generated by `expo start`, is
gitignored, and was left over from an older tree. CI has no such file, so routes
are untyped there and this never fails in CI.
