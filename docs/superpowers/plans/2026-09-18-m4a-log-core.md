...
BEGIN
  ...
  VALUES (
    ...
    v_day.label, v_day.day_number, v_day.week_number
  )
...
```
Expected: `131` passes (95 before + 36 new) and the last line `ROLLBACK`. If a count differs by one or two, read the pass list — the number is a check, not the contract.
...
|---|---|---|
| SQL | `psql … -f db/rls_assertions.sql` | ends in `ROLLBACK`, ~131 `pass` notices, zero `FAIL` |
