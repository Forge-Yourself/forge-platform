...
Executed 2026-09-18/19 by subagent-driven development, one agent per task with a
spec-compliance diff and a code review between them. Every task committed
separately on `develop`; the SHAs are in the git log between `4481ca6` and the
M4a close-out.

### Changed while implementing

- **Task 2/3 — `log_set` PR array.** `v_prs || 'weight'` resolves to
  `anyarray || anyarray`, so Postgres tried to parse `'weight'` as an array
  literal and every PR-earning insert raised `malformed array literal`. That is
  every client's first working set on every exercise. Now `array_append`. The
  harness caught it before the migration was pushed.
- **Task 1 — audit assertions read as the table owner.** `audit_logs` has RLS on
  and no policies, so `pg_temp.expect` counting a `workout_start` row as
  `authenticated` saw 0. The M2 block already had this convention; the M4a block
  now follows it.
- **Task 3 — `start_workout_session` uses scalars, not a `RECORD`.** An
  unassigned `RECORD` raises the moment it is read, which is exactly the
  freestyle path (`p_program_day_id IS NULL`). Three scalars and `IF NOT FOUND`.
- **Task 3 — `duration_min` is clamped.** `GREATEST(1, …)::SMALLINT` overflows
  after ~22.7 days, and since a stale session is only ever closed by whoever
  opens it next, an overflow would make the client unable to log again, ever.
  Now `LEAST(32767, …)`.
- **Task 3 — starts are serialised per client.** `FOR UPDATE` on a row that does
  not exist yet locks nothing, so a PT and a client opening the app together
  could both insert an `in_progress` session and defeat D4. A
  `PERFORM 1 FROM clients … FOR UPDATE` before the lookup fixes it without a new
  index.
- **Task 3 — the program-day lookup requires `state IN ('active','completed')`**
  so it cannot snapshot a label off a draft the client is not allowed to see.
- **Task 2 — every legacy partition gets the write revoke**, not just `sets_*`.
  The live project was carrying `*_2026_11 … *_2027_06` partitions that no
  migration created, with RLS off and full `authenticated` write grants.
  `0015` adopts and closes them. This is the repo/DB drift the spec predicted.
- **Tasks 8/9 — no `?? undefined` on `log_set` args.** PostgREST resolves an RPC
  overload by the set of argument names, so omitting `p_weight_kg` for a
  bodyweight set would fail with PGRST202 rather than sending SQL NULL. The
  wrapper sends the null and carries a one-line cast with the reason.
- **Task 9 — set counts are HEAD counts.** Counting by pulling one row per set
  across 50 sessions silently truncates at PostgREST's `max_rows` (1000). One
  `count: 'exact', head: true` per session instead, in both the mobile hook and
  the admin page.
- **Task 9 — offline is `code === ''` plus a network message**, not
  `code === undefined`. postgrest-js reports a failed fetch as an empty code
  with the `TypeError` text in the message; `undefined` instead matched a 502
  HTML body and the wrapper's own `Error`. Misclassifying a server fault as
  offline would, in M4b, route a set into the outbox that the server rejected.
- **Task 9 — `nullsFirst: false`** on every `started_at` ordering. The column is
  nullable and Postgres sorts NULLS FIRST on DESC, so M5's `programmed` rows
  would otherwise head the history.
- **Task 9 — `nextSetNumber` is `max + 1`**, not `count + 1`, so deleting a
  middle set cannot produce two sets numbered the same.
- **Task 12 — the header reads the session's own snapshot** (`day_label`,
  `week_number`, `day_number`) rather than `day.label`, so a rebuilt program
  week or an unreadable program still renders "Week 1 · Day 1".
- **Task 12 — the PR banner trusts `log_set`'s returned `new_prs` only.**
  `detectPrs` stays the tested TS mirror of the rule, not a second opinion at
  runtime.
- **Task 5 — no `ulidx` dependency.** A 40-line Crockford base32 encoder in
  `packages/shared` with an injected random source, wrapped by `expo-crypto` in
  the app, so the encoder is unit-tested without a device.
- **Task 10 — `useState(() => Date.now())`.** The React Compiler's purity rule
  rejects the inline call during render.

### Verified

- RLS harness: **130 assertions, 0 failures**, against the live Frankfurt
  project, ending in `ROLLBACK`.
- `packages/shared`: **261 tests**, 13 files.
- `turbo run typecheck lint test`: clean across all three packages.
- Screen walk (`forge-screen-walk`, `walk-m4a.mjs`): **15/15 steps**, both
  personas, zero page errors and zero raw i18n keys. Screenshots reviewed.
- Live DB after the walk: per-set `logged_by_user_id` distinguishes PT from
  client, `is_pt_led` matches who started, day snapshots present, `is_synced`
  true on every row, PR rows point at their set, and `workout_start` /
  `workout_complete` audit rows exist.
- Admin web: the client user page lists the sessions and the detail page renders
  every set with its logger and PR flags.

### Known limitations, by design

- The rest timer stops if the app is killed; lock-screen persistence is M4d.
- Editing or deleting a set never re-evaluates PRs, so an `exercise_prs` row can
  outlive the set that earned it.
- There is no abandon-session action: a stale `in_progress` session is completed
  by whoever opens it next.
- The audio cue is unverified on a real device — it is web-guarded and wrapped
  in try/catch, and the haptic is the signal that matters.

### Discovered, not caused by this milestone

- `admin.test@forge.dev` did not share the documented test password; it was
  reset to the shared one so the admin pages could be verified.
