...
- **Phase:** In development. M0 (foundation), M1 (auth & identity), M2 (clients & intake), M3 (programming), M4a (log core), M4b (offline + live mirror), M4c (body metrics and progress photos), M4d (floor ergonomics: iPad console, voice, lock-screen rest) and M4e (a PT trains themselves) are complete. M5 (scheduling) is next. M4 was split into four sub-milestones on 2026-09-18 — see the M4a spec for why.
...
| `docs/superpowers/plans/2026-09-20-m4e-self-client.md` | M4e: a PT trains themselves. Rules that outlive M4e: every schema predicate is `is_pt_of_client() OR is_client_record_owner()`, so a self row satisfies both and needs no new policy; the self row is split out of `useClientList.items` because `usePtDashboard` counts them as neglected clients otherwise; and an outbox that names a column the RPC infers (`is_pt_led`) must agree with it — PITFALLS O3 |
...
| M4e | Self-training — a PT is their own client: `ensure_self_client()`, /me, roster pinning | ✅ done |
